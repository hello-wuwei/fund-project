  ...
  export default {
    ...
    created () {
      if (this.row) {
        ...
        let {userName, loginName, userState, idCard} = this.row
        this.submitData.userName = userName
        this.submitData.loginName = loginName
        this.submitData.userState = userState
        this.submitData.idCard = idCard
      }
    },
    methods: {
      ...
      onSubmit (form) {
        this.$refs[form].validate(async (valid) => {
          if (valid) {
            ...
            let response = await this.$sever.updateChildAccount(params)
            if (response.code === 0) {
//                this.$emit('refresh')
              this.$message({
                message: '子账号修改成功!',
