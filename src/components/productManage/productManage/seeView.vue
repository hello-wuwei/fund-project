...
<template lang="html">
  <div class="page">
    <com-breadcrumb :list="['产品创设', '查看']"></com-breadcrumb>
      ...
      <div class="border-line">
        ...
        <el-form class="form-list" label-width="240px" style="width: 700px">
          ...
          <el-form-item label="客户签署合同是否需要录音录像：">
            <el-radio v-for="(item, index) in productInfo.videoStateList" :key="index" v-model="item.isChecked" disabled label="1">{{item.description}}</el-radio>
          </el-form-item>
        </el-form>
        ...
        <h4 style="padding-top: 30px; margin-top: 20px; border-top:1px solid #ccc">合同信息</h4>
...
<script>
  ...
  export default {
    ...
    methods: {
      async getProInfo () {
        ...
        switch (res.code) {
          case 0:
            ...
            res.data.bean.videoStateList.forEach((item) => {
              item.isChecked = item.isChecked.toString()
            })
      ...
      async getProRiskInfo () {
        let res = await this.$sever.getProRiskInfo({bookId: this.propValue.bookId, sysCode: 'TTD_MANAGE_SYSTEM'})
        switch (res.code) {
          case 0:
            this.productInfo_risk = res.data.bean
            this.productInfo_risk.revealBookUrl = JSON.parse(this.productInfo_risk.revealBookUrl)
