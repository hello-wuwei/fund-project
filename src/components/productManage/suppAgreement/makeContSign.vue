<template>
  <div class="page">
    <com-breadcrumb :list="['补充协议', '文件处理', '打标签']"></com-breadcrumb>
          ...
          <div style="min-width: 1260px;overflow: hidden">
            ...
            <div class="contract-con">
              ...
              <div class="pdf-box" style="position:relative;width: 892.5px;height:1261.5px;">
                <pdf-view :src="productInfo.suppleContractUrl.url" :page="page" @numPages="numPages = $event">
    ...
    <el-dialog title="提 示" :visible.sync="sureBack" style="margin:auto;width: 700px">
      <div style="text-align: center">您是否确认打标签设置完成？确认后补充协议将发送至管理人。</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getViewLabel () {
        viewSignFile(3, this.productInfo.productId, this.propValue.contractId, this.labelData, this)
      },
      ...
      async getProInfo () {
        ...
        switch (res.code) {
          case 0:
            ...
            this.productInfo.suppleContractUrl = JSON.parse(res.data.bean.suppleContractUrl)
            let labelContent = await getLabelViewData(this.productInfo.productId, this.productInfo.contractId, 0, 3, this)
...
//          this.$message({message: '您所打标签中缺少投资机构签章标签，无法提交', type: 'warning'})
//          return false
//        }
//        if (!this.labelData.some((item) => { return parseInt(item.labelType) === 11 })) {
//          this.$message({message: '您所打标签中缺少管理人公章标签，无法提交', type: 'warning'})
//          return false
//        }
//        if (!this.labelData.some((item) => { return parseInt(item.labelType) === 15 })) {
//          this.$message({message: '您所打标签中缺少托管人公章标签，无法提交', type: 'warning'})
//          return false
//        }
//        if (!this.labelData.some((item) => { return parseInt(item.labelType) === 23 })) {
//          this.$message({message: '您所打标签中缺少管理人法人章标签，无法提交', type: 'warning'})
//          return false
//        }
//        if (!this.labelData.some((item) => { return parseInt(item.labelType) === 25 })) {
//          this.$message({message: '您所打标签中缺少托管人法人章标签，无法提交', type: 'warning'})
//          return false
//        }
//
//        // 文本标签限制
//        if (!this.labelData.some((item) => { return item.showName === '身份证号' })) {
//          this.$message({message: '您所打标签中缺少身份证号标签，无法提交', type: 'warning'})
//          return false
//        }
//        if (!this.labelData.some((item) => { return item.showName === '签署日期' })) {
//          this.$message({message: '您所打标签中缺少签署日期标签，无法提交', type: 'warning'})
//          return false
//        }
//        if (!this.labelData.some((item) => { return item.showName === '认购/申购金额（大写）' })) {
//          this.$message({message: '您所打标签中缺少认购（申购）金额（大写）标签，无法提交', type: 'warning'})
//          return false
//        }
        return true
      },
      ...
      async subLabel () {
        ...
        let obj = {
          ...
          targetId: this.propValue.contractId,
          filePdfUrl: this.productInfo.suppleContractUrl,
