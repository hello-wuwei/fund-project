<template lang="html">
  <div class="page">
    <com-breadcrumb :list="['标签管理']"></com-breadcrumb>
    <div class="content-main">
      <el-table header-row-class-name="table-header-define" class="table-btn-style" :data="tableData" highlight-current-row style="width: 100%">
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button v-if="PROD_LABEL_USED" size="mini" type="primary" @click="doneUse(scope.row.labelId,scope.row.isUsed)">{{{0:'启用',1:'停用'}[scope.row.isUsed]}}</el-button>
          </template>
        </el-table-column>
      </el-table>
