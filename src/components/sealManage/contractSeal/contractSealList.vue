<template lang="html">
  <div class="page">
    <com-breadcrumb :list="['管理人合同章管理']"></com-breadcrumb>
    <div class="content-main">
      <el-form :inline="true" class="demo-form-inline">
        <el-form-item label="关键词">
          <el-input v-model="formData.keyword" placeholder="请输入托管人名称" size="small"></el-input>
        </el-form-item>
      ...
      <el-table header-row-class-name="table-header-define" class="table-btn-style" :data="tableData" highlight-current-row style="width: 100%">
        ...
        <el-table-column label="操作" align="center">
          <template v-if="SEAL_CONT_UPLOAD" slot-scope="scope">
            <el-button size="mini" v-if="scope.row.state==3" type="primary" @click="editSeal(scope.row.userId)">编 辑</el-button>
            <el-button size="mini" v-if="scope.row.state==1" type="primary" @click="uploadSeal(scope.row.userId)">上 传</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      async getDataList () {
        ...
        let res = await this.$sever.getManagerSealList(this.formData)
        switch (res.code) {
          case 0:
      ...
      editSeal (userId) {
        this.$router.push({path: '/home/contractSeal/upload', query: {userId: userId}})
      },
      uploadSeal (userId) {
        this.$router.push({path: '/home/contractSeal/upload', query: {userId: userId}})
      },
