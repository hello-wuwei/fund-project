<template lang="html">
  <div class="page">
    ...
    <el-tabs style="border: none" v-model="activeTag" type="card" closable @tab-remove="removeTag">
      ...
      <component :row="propValue" v-on:addTag="addTag" v-on:removeTag="removeTag" :is='activeTag' keep-alive></component>
    </el-tabs>
  </div>
</template>

<script>
  import operLogList from './operLog/operLogList.vue'
  import watchLog from './operLog/watchLog.vue'
  export default {
    components: {
      operLogList,
      watchLog
    ...
    data () {
      return {
        // tab组件切换
        comNameConfig: 'operLogList',         // 菜单主页面组件名称（固定值/常量）
        activeTag: 'operLogList',
        editableTabs: [{
          title: '操作日志',
          name: 'operLogList'
        }],
    ...
    methods: {
      ...
      addTag(data) {
        ...
        this.propValue = data.row ? data.row : this.propValue
      },
      removeTag(name) {
