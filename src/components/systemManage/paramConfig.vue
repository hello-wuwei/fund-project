<template lang="html">
  <div class="page">
    ...
    <el-tabs style="border: none" v-model="activeTag" type="card" closable @tab-remove="removeTag">
      ...
      <component :id="propValue" v-on:addTag="addTag" v-on:removeTag="removeTag" :is='activeTag' keep-alive></component>
    </el-tabs>
  </div>
</template>

<script>
  import paramConfigList from './paramConfig/paramConfigList.vue'
  import editParam from './paramConfig/editParam.vue'
  export default {
    components: {
      paramConfigList,
      editParam
    ...
    data () {
      return {
        // tab组件切换
        comNameConfig: 'paramConfigList',         // 菜单主页面组件名称（固定值/常量）
        activeTag: 'paramConfigList',
        editableTabs: [{
          title: '参数配置',
    ...
    methods: {
      ...
      addTag(data) {
        ...
        this.propValue = data.id ? data.id : this.propValue
      },
      removeTag(name) {
