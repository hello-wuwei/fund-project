  ...
  <div class="page">
    ...
    <div class="content-main">
      ...
      <el-table header-row-class-name="table-header-define" :data="tableData" highlight-current-row style="width: 100%">
        ...
        <el-table-column property="roleInfo" label="用户角色" align="center"></el-table-column>
        ...
        <el-table-column property="lastLoginIp" label="最后登录IP" align="center"></el-table-column>
        ...
        <el-table-column label="操作" align="center" width="400">
          <template slot-scope="scope">
            <div v-if="!scope.row.isAdmin">
              <el-button size="mini" type="primary" @click="getRole_system(scope.row.userId)">授 权</el-button>
              <el-button size="mini" type="primary" @click="editAccount(scope.row.userId)">修 改</el-button>
              <el-button size="mini" @click="deleteAlert(scope.row.userId)">删 除</el-button>
              <el-button size="mini" @click="stopUsed(scope.row.userId,scope.row.state)">{{scope.row.state==3?'停 用':'启 用'}}</el-button>
              <el-button size="mini" @click="resetPsd(scope.row.userId)">密码重置</el-button>
            </div>
            <div v-else>- - -</div>
          </template>
        </el-table-column>
    ...
    <el-dialog title="授权" :visible.sync="dialogShow" width="800px">
      <div class="con-box">
        <el-checkbox-group v-model="curRoleIds">
          <el-checkbox v-for="(role,index) in rolelist" v-if="!role.isAdmin" :label="role.roleId" :key="index">{{role.name}}</el-checkbox>
        </el-checkbox-group>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      editAccount (userId) {
        this.$emit('addTag', {title: '修改账号', name: 'editAccount', userId: userId})
      },
      ...
      resetPsd (userId) {
        this.$confirm('确定重置该用户密码为“smqy123”吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
