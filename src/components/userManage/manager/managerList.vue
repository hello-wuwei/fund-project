<template lang="html">
  <div class="page">
    <com-breadcrumb :list="['管理人管理']"></com-breadcrumb>
    <div class="content-main">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <!--<el-form-item label="数字密码">-->
          <!--<el-select v-model="formData.passwordState" placeholder="请选择数字密码状态" size="small">-->
            <!--<el-option label="全部" value=""></el-option>-->
      ...
      <el-table header-row-class-name="table-header-define" class="table-btn-style" :data="tableData" highlight-current-row style="width: 100%">
        ...
        <el-table-column property="stateAccount" label="账户认证" align="center">
          <template slot-scope="scope">
            <i :style="stateColor(scope.row.stateAccount)">{{accAuthState(scope.row)}}</i>
          </template>
        </el-table-column>
        ...
        <el-table-column property="roleInfo"show-overflow-tooltip label="所属角色" :formatter="em" align="center"></el-table-column>
        <el-table-column property="created" label="创建时间" align="center" width="160">
          <template slot-scope="scope">{{scope.row.created | date}}</template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="300">
          <template slot-scope="scope">
            <el-button size="mini" v-if="USER_MANA_ORGCHECK && scope.row.viewState==0" type="primary" @click="orgAppoint(scope.row.userId)">机构审核</el-button>
            <!--<el-button size="mini" v-if="USER_MANA_ACCCHECK && scope.row.stateAccount==0" type="primary" @click="accAppoint(scope.row.userId)">账户审核</el-button>-->
            <el-button size="mini" v-if="USER_MANA_ROOT && scope.row.state==3" type="primary" @click="getRole_user(scope.row.userId)">授 权</el-button>
            <el-button
              size="mini"
              v-if="USER_MANA_SEE && (scope.row.viewState==1 || scope.row.stateAccount==1 || scope.row.viewState==3 || scope.row.stateAccount==3)"
              type="primary"
              @click="seeInfo(scope.row.userId)">查 看</el-button>
            <el-button v-if="USER_MANA_USED" size="mini" @click="disabledUse(scope.row.userId,scope.row.state)">{{scope.row.state==1?'启 用':'停 用'}}</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getAllDataList () {
        ...
        this.formData.passwordState = ''
        this.formData.createdStart = ''
        this.formData.createdEnd = ''
        this.getDataList()
      },
      seeInfo (userId) {
        this.$router.push({path: '/home/manager/see', query: {userId: userId}})
      },
      orgAppoint (userId) {
        this.$router.push({path: '/home/manager/orgCheck', query: {userId: userId}})
      },
      accAppoint (userId) {
        this.$router.push({path: '/home/manager/accCheck', query: {userId: userId}})
      },
