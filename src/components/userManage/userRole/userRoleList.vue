  ...
  <div class="page">
    ...
    <div class="content-main">
      ...
      <el-table header-row-class-name="table-header-define" class="table-btn-style" :data="tableData" highlight-current-row style="width: 100%">
        ...
        <el-table-column property="description" label="角色描述" :formatter="em" align="center"></el-table-column>
        ...
        <el-table-column label="操作" align="center" width="400">
          <template slot-scope="scope">
            <el-button v-if="USER_ROLE_ROOT" size="mini" type="primary" @click="getAuth(scope.row.roleId)">授 权</el-button>
            <el-button v-if="USER_ROLE_EDIT" size="mini" type="primary" @click="editAccount(scope.row.roleId)">修 改</el-button>
            <el-button v-if="USER_ROLE_SEE" size="mini" type="primary" @click="seeAccount(scope.row.roleId)">查 看</el-button>
            <el-button v-if="USER_ROLE_DELE" size="mini" @click="deleteAlert(scope.row.roleId)">删 除</el-button>
            <el-button v-if="USER_ROLE_USED" size="mini" @click="stopUsed(scope.row.roleId,scope.row.isUsed)">{{scope.row.isUsed==1?'停 用':'启 用'}}</el-button>
          </template>
        </el-table-column>
      </el-table>

      <div class="foot-bottom">
        <el-button v-if="USER_ROLE_NEW" class="bom-btn" size="mini" icon="el-icon-plus" @click="addAccount">新增角色</el-button>
        <el-pagination
          style="float: right"
...
<script>
  export default {
    ...
    methods: {
      ...
      addAccount () {
        this.$router.push({path: '/home/userRole/add'})
      },
      editAccount (roleId) {
        this.$router.push({path: '/home/userRole/edit', query: {roleId: roleId}})
      },
      seeAccount (roleId) {
        this.$router.push({path: '/home/userRole/see', query: {roleId: roleId}})
      },
